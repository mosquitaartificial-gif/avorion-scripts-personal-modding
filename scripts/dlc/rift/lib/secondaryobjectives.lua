        ...
        return {
            ...
            postGeneration = function(self, specs)
                ...
                for _, wreckage in pairs(cargoWreckages) do
                    ...
                    wreckage:setValue("secondary_objective_wreckage", true)
                    wreckage:addScriptOnce("utility/aiundockable.lua")
                end
...
function SecondaryObjectives.getData()
    local objectives = {}
    for v, _ in pairs(SecondaryObjectives.Type) do
        local type = SecondaryObjectives[SecondaryObjectives.Type[v]]
        local data = {
            icon = type.icon,
            title = type.name
        }

        objectives[tostring(v)] = data
    end

...
function SectorTemplate.contents(x, y)
    local seed = Seed(string.join({GameSeed(), x, y, "defenderasteroidfield"}, "-"))
    math.randomseed(seed);

    local random = random()

    local contents = {stations = 0, seed = tostring(seed)}
    ...
    contents.asteroidEstimation = generator:estimateAsteroidNumbers(3 + contents.claimableAsteroids, 8)

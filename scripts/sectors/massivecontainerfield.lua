...
function SectorTemplate.contents(x, y)
    local seed = Seed(string.join({GameSeed(), x, y, "massivecontainerfield"}, "-"))
    ...
    contents.asteroidEstimation = generator:estimateAsteroidNumbers(6 + contents.claimableAsteroids, 5)
...
function SectorTemplate.generate(player, seed, x, y)
    ...
    local numFields = random:getInt(4, 8)
    for i = 1, numFields do
        generator:createAsteroidField(0.075)
    end

    local numSmallFields = random:getInt(4, 6)

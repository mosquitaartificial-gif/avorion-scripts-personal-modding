...
function SectorGenerator:createContainerField(sizeX, sizeY, circular, position, factionIndex, hackables, plan)
    ...
    local plan = plan or PlanGenerator.makeContainerPlan()
...
--    profiler:section("framework")
    ...
    for i, index in pairs(indices) do
        if i % 3 == 0 then
            ...
            plan:addBlock(box.position + offset, size, index, -1, color, material, Matrix(), BlockType.Framework, ColorNone())
            plan:addBlock(box.position - offset, size, index, -1, color, material, Matrix(), BlockType.Framework, ColorNone())
        end
    end

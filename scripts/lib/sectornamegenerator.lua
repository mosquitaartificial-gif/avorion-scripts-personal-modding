...
local SectorNameGenerator = {}
function SectorNameGenerator.gridDimensions(x, y)
...
function SectorNameGenerator.ringIndexToGrid(index)
    -- top bar
    if index < 6 then
        return -3 + index, -3
    end
...
function SectorNameGenerator.generateSectorName(x, y, count, seed)
    ...
    local arwX, arwY = SectorNameGenerator.ringIndexToGrid(arwSector)
    local dskoX, dskoY = SectorNameGenerator.ringIndexToGrid(dskoSector)

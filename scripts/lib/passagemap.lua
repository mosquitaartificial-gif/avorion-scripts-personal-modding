...
local function new(seed)
    ...
    obj:initialize(seed, -499, 500, GameSettings().rifts)
...
function PassageMap:passable(x, y)
    ...
    local min = Balancing.BlockRingMin or 150
    local max = Balancing.BlockRingMax or 152

    if d2 > min * min and d2 < max * max then
        if GameSettings().barrier then
            return false
        end
    end

    ...
    for _, coords in pairs(coordinates) do
        ...
        if distance >= mind and distance < maxd then
            ...
            if offgrid and not regular and not blocked and not home then
                ...
                local contents = specs.generationTemplate.contents(coords.x, coords.y)
                -- we don't want a pirate encounter to fight against the 4
                if string.match(specs:getScript(), "wreckagefield") and contents.pirateEncounter == false then

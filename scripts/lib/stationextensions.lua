...
function addConstructionScaffold(entity)
    ...
    for i = 0, 10 do
        ...
        lmid = scaffold:addBlock(p + vec3(0, 0, 0), mlong, bmid, -1, c, m, o, blank, ColorNone())
        bmid = scaffold:addBlock(p + vec3(0, 0, hmlong.z + hmblock.z), mblock, lmid, -1, c, m, o, blank, ColorNone())


        local len = 5.0 - hmblock.x * 0.5
        bmid = scaffold:addBlock(p + vec3(-hmblock.x - len * 0.5, 0, hmlong.z + hmblock.z), vec3(len, middleDiameter, middleDiameter), lmid, -1, c, m, o, blank, ColorNone())
        ...
        for j = 0, 4 do -- create a tilted bar from edge blocks
            l = scaffold:addBlock(p + vec3(longa.x * j, longa.y * j,        0), longa, l, -1, c, m, MatrixLookUp(vec3(-1, 0, 0), vec3(0, -1, 0)), edge, ColorNone())
            l = scaffold:addBlock(p + vec3(longa.x * j, longa.y * (j + 1),  0), longa, l, -1, c, m, MatrixLookUp(vec3(1, 0, 0), vec3(0, 1, 0)), edge, ColorNone())
        end

        p = p + vec3(longa.x * 4.5, longa.y * 5, 0)

        l = scaffold:addBlock(p + vec3(blocka.x * 0.5, 0, 0), blocka, l, -1, c, m, o, blank, ColorNone())

        p = p + vec3(blocka.x * 0.5, blocka.y * 0.5 + longb.y * 0.5, 0)

        for j = 0, 2 do -- create a tilted bar from edge blocks
            l = scaffold:addBlock(p + vec3(longb.x * j,         longb.y * j, 0), longb, l, -1, c, m, MatrixLookUp(vec3(1, 0, 0), vec3(0, 1, 0)), edge, ColorNone())
            l = scaffold:addBlock(p + vec3(longb.x * (j + 1),   longb.y * j, 0), longb, l, -1, c, m, MatrixLookUp(vec3(-1, 0, 0), vec3(0, -1, 0)), edge, ColorNone())
        end

        p = p + vec3(longb.x * 3, longb.y * 2.5, 0)

        l = scaffold:addBlock(p + vec3(0, blockb.y * 0.5, 0), blockb, l, -1, c, m, o, blank, ColorNone())

        p = p + vec3(0, blockb.y + longc.y * 0.5, 0)

        for j = 0, 1 do -- create a tilted bar from edge blocks
            l = scaffold:addBlock(p + vec3(-longc.x * j,         longc.y * j, 0), longc, l, -1, c, m, MatrixLookUp(vec3(-1, 0, 0), vec3(0, 1, 0)), edge, ColorNone())
            l = scaffold:addBlock(p + vec3(-longc.x * (j + 1),   longc.y * j, 0), longc, l, -1, c, m, MatrixLookUp(vec3(1, 0, 0), vec3(0, -1, 0)), edge, ColorNone())
        end
    end
    ...
    local connector = scaffold:addBlock(vec3(0, 0, 0), vec3(10 + middleDiameter, middleDiameter, middleDiameter), 0, -1, c, m, o, blank, ColorNone())
    local root = scaffold:addBlock(vec3(0, 0, 0), mblock, connector, -1, c, m, o, blank, ColorNone())
...
function addProductionCenters(entity, numAdditions, sizeX, sizeY)
    ...
    for _, box in pairs({box1, box2}) do
        ...
        if panelType == 0 then
            ...
            local pi = box:addBlock(vec3(0, 0, 0), panel, -1, -1, c, m, o, bottom, ColorNone())
            local ui = box:addBlock(vec3(0, 0, panel.z * 0.5 + glow.z * 0.5), glow, pi, -1, glowColor, m, o, BlockType.Glow, ColorNone())
            local ui = box:addBlock(vec3(0, 0, panel.z * 0.5 + glow.z + upper.z * 0.5), upper, ui, -1, c, m, o, bottom, ColorNone())

        elseif panelType == 1 then

            local pi = box:addBlock(vec3(0, 0, 0), panel, -1, -1, c, m, o, bottom, ColorNone())
            local ui = box:addBlock(vec3(0, 0, panel.z * 0.5 + glow.z * 0.5), glow, pi, -1, glowColor, m, o, BlockType.Glow, ColorNone())
            local ui = box:addBlock(vec3(0, 0, panel.z * 0.5 + glow.z * 1.5), glow, ui, -1, c, m, o, bottom, ColorNone())

            local u = upper * vec3(0.25, 0.25, 1.0)
            local z = panel.z * 0.5 + glow.z * 2.0 + u.z * 0.5

            box:addBlock(vec3(panel.x * 0.25, panel.y * 0.25, z), u, ui, -1, c, m, o, bottom, ColorNone())
            box:addBlock(vec3(-panel.x * 0.25, panel.y * 0.25, z), u, ui, -1, c, m, o, bottom, ColorNone())
            box:addBlock(vec3(-panel.x * 0.25, -panel.y * 0.25, z), u, ui, -1, c, m, o, bottom, ColorNone())
            box:addBlock(vec3(panel.x * 0.25, -panel.y * 0.25, z), u, ui, -1, c, m, o, bottom, ColorNone())
        ...
        elseif panelType == 2 then
            ...
            box:addBlock(vec3(panel.x * 0.25, panel.y * 0.25, z), u, ui, -1, c, m, MatrixLookUp(vec3(-1, 0, 0), vec3(0, 0, 1)), corner, ColorNone())
            box:addBlock(vec3(-panel.x * 0.25, panel.y * 0.25, z), u, ui, -1, c, m, MatrixLookUp(vec3(0, -1, 0), vec3(0, 0, 1)), corner, ColorNone())
            box:addBlock(vec3(-panel.x * 0.25, -panel.y * 0.25, z), u, ui, -1, c, m, MatrixLookUp(vec3(1, 0, 0), vec3(0, 0, 1)), corner, ColorNone())
            box:addBlock(vec3(panel.x * 0.25, -panel.y * 0.25, z), u, ui, -1, c, m, MatrixLookUp(vec3(0, 1, 0), vec3(0, 0, 1)), corner, ColorNone())
    ...
    for i = 0, rows - 1 do
        ...
        mid = panels:addBlock(p + vec3(0, 0, 0), middle, mid, -1, c, m, o, hull, ColorNone())
        local s1 = panels:addBlock(p + vec3(-side.x * 0.5, 5.0, 0), side, mid, -1, c, m, o, hull, ColorNone())
        local s2 = panels:addBlock(p + vec3(side.x * 0.5, 5.0, 0), side, mid, -1, c, m, o, hull, ColorNone())
        ...
        for j = 1, additionals do
            ...
            s2 = panels:addBlock(offset + vec3(side.x * 0.5, 5.0, 0), side, p2, -1, c, m, o, hull, ColorNone())
            p1 = panels:addPlanDisplaced(s1, box, 0, offset + vec3(side.x + panel.x * 0.5, 5.0, 0))

            local offset = p + vec3(-(panel.x + side.x) * j, 0, 0)

            s1 = panels:addBlock(offset + vec3(-side.x * 0.5, 5.0, 0), side, p1, -1, c, m, o, hull, ColorNone())
            p1 = panels:addPlanDisplaced(s1, box, 0, offset + vec3(-side.x - panel.x * 0.5, 5.0, 0))
...
function addFarmingCenters(entity, arms, sizeX, sizeY)
    ...
    for i = 0, rows - 1 do
        ...
        local p1 = panels:addBlock(p + vec3(-side.x - panel.x * 0.5, 5.0, 0), panel, s1, -1, c, m, o, solar, ColorNone())
        local p2 = panels:addBlock(p + vec3(side.x + panel.x * 0.5, 5.0, 0), panel, s2, -1, c, m, o, solar, ColorNone())

        panels:addBlock(p + vec3(-side.x - panel.x * 0.5, 5.0, panel.z * 0.5 + glass.z * 0.5), glass, s1, -1, c, m, o, BlockType.Glass, ColorNone())
        panels:addBlock(p + vec3(side.x + panel.x * 0.5, 5.0, panel.z * 0.5 + glass.z * 0.5), glass, s2, -1, c, m, o, BlockType.Glass, ColorNone())

        for j = 1, additionals do
            local offset = p + vec3((panel.x + side.x) * j, 0, 0)

            s2 = panels:addBlock(offset + vec3(side.x * 0.5, 5.0, 0), side, p2, -1, c, m, o, hull, ColorNone())
            p2 = panels:addBlock(offset + vec3(side.x + panel.x * 0.5, 5.0, 0), panel, s2, -1, c, m, o, solar, ColorNone())

            panels:addBlock(offset + vec3(side.x + panel.x * 0.5, 5.0, panel.z * 0.5 + glass.z * 0.5), glass, s2, -1, c, m, o, BlockType.Glass, ColorNone())

            local offset = p + vec3(-(panel.x + side.x) * j, 0, 0)

            s1 = panels:addBlock(offset + vec3(-side.x * 0.5, 5.0, 0), side, p1, -1, c, m, o, hull, ColorNone())
            p1 = panels:addBlock(offset + vec3(-side.x - panel.x * 0.5, 5.0, 0), panel, s1, -1, c, m, o, solar, ColorNone())

            panels:addBlock(offset + vec3(-side.x - panel.x * 0.5, 5.0, panel.z * 0.5 + glass.z * 0.5), glass, s1, -1, c, m, o, BlockType.Glass, ColorNone())
...
function addSolarPanels(entity, arms)
    ...
    local mid = panels:addBlock(vec3(0, 0, 0), middle * 1.2, mid, -1, c, m, o, hull, ColorNone())

    for i = 1, rows do
        local p = vec3(0, i * 20, 0)

        mid = panels:addBlock(p + vec3(0, 0, 0), middle, mid, -1, c, m, o, hull, ColorNone())
        local s1 = panels:addBlock(p + vec3(-2.5, 5.0, 0), side, mid, -1, c, m, o, hull, ColorNone())
        local s2 = panels:addBlock(p + vec3(2.5, 5.0, 0), side, mid, -1, c, m, o, hull, ColorNone())

        local p1 = panels:addBlock(p + vec3(-5 - panel.x * 0.5, 5.0, 0), panel, s1, -1, c, m, o, solar, ColorNone())
        local p2 = panels:addBlock(p + vec3(5 + panel.x * 0.5, 5.0, 0), panel, s2, -1, c, m, o, solar, ColorNone())


        for j = 1, additionals do
            local offset = p + vec3((panel.x + side.x) * j, 0, 0)

            s2 = panels:addBlock(offset + vec3(2.5, 5.0, 0), side, p2, -1, c, m, o, hull, ColorNone())
            p2 = panels:addBlock(offset + vec3(5 + panel.x * 0.5, 5.0, 0), panel, s2, -1, c, m, o, solar, ColorNone())

            local offset = p + vec3(-(panel.x + side.x) * j, 0, 0)

            s1 = panels:addBlock(offset + vec3(-2.5, 5.0, 0), side, p1, -1, c, m, o, hull, ColorNone())
            p1 = panels:addBlock(offset + vec3(-5 - panel.x * 0.5, 5.0, 0), panel, s1, -1, c, m, o, solar, ColorNone())
        end
    end
...
function addCargoStorage(entity, numAdditions, sizeX, sizeY)
    ...
    local pi = box:addBlock(vec3(0, 0, 0), cargoSize, -1, -1, c, m, o, BlockType.CargoBay, ColorNone())

    for i = 0, rows - 1 do
        local p = vec3(0, i * spacing, 0)

        mid = containers:addBlock(p + vec3(0, 1.5, 0), mainBridgeSize, mid, -1, c, m, o, hull, ColorNone())
        local s1 = containers:addBlock(p + vec3(-bridgeSize.x * 0.5, 5.0, 0), bridgeSize, mid, -1, c, m, o, hull, ColorNone())
        local s2 = containers:addBlock(p + vec3(bridgeSize.x * 0.5, 5.0, 0), bridgeSize, mid, -1, c, m, o, hull, ColorNone())
        ...
        for j = 1, additionals do
            ...
            s2 = containers:addBlock(offset + vec3(bridgeSize.x * 0.5, 5.0, 0), bridgeSize, p2, -1, c, m, o, hull, ColorNone())
            p1 = containers:addPlanDisplaced(s1, box, 0, offset + vec3(bridgeSize.x + cargoSize.x * 0.5, 5.0, 3.0))

            local offset = p + vec3(-(cargoSize.x + bridgeSize.x) * j, 0, 0)

            s1 = containers:addBlock(offset + vec3(-bridgeSize.x * 0.5, 5.0, 0), bridgeSize, p1, -1, c, m, o, hull, ColorNone())
            p1 = containers:addPlanDisplaced(s1, box, 0, offset + vec3(-bridgeSize.x - cargoSize.x * 0.5, 5.0, 3.0))

...
function BigAICorrupted.update(timestep)
    ...
    if done then
        ...
        for _, player in pairs({Sector():getPlayers()}) do
            player:sendCallback("onCorruptedAIDestroyed")
        end
    end
end
...
function BigAICorrupted.onSectorLeft(playerId, x, y, changeType)
    terminate() -- if player runs, he shouldn't be able to respawn Corrupted AI on reenter
end
...
function BigAICorrupted.addTurrets(boss, numTurrets)
    ...
    local turret = generator:generate(0, 0, 0, Rarity(RarityType.Exceptional), WeaponType.PlasmaGun)
    ...
    turret = generator:generate(0, 0, 0, Rarity(RarityType.Exceptional), WeaponType.RailGun)
...
function BigAICorrupted.spawn(x, y)
    ...
    local plan = LoadPlanFromFile("data/plans/big_ai_corrupted.xml")
    ...
    BigAICorrupted.addTurrets(boss, 100)

...
mission.data.brief = "The Haatii"%_T
mission.data.title = "The Haatii"%_T
...
{
    "Find out more about the Haatii"%_T,
...
mission.phases[1].onBeginServer = function()
    ...
    mail.header = "The Haatii /* Mail Subject */"%_T
...
function findNearbyBeacon(x_in, y_in)
    ...
    local x, y
    for i = 0, 5 do
        x, y = MissionUT.getSector(x_in, y_in, 8, 25 + i * 10, false, false, false, false, false)
        if x and y then
            break
        end
    end
    ...
    if not x or not y then
        x, y = MissionUT.getSector(0, 0, 350, 500, false, false, false, false, false)
    end

...
function findEmptySectorNearby(x_in, y_in)
    local x, y

    for i = 0, 10 do
        x, y = MissionUT.getSector(x_in, y_in, 3, 7 + i * 10, false, false, false, false, false)
        if x and y then
            break
        end
    end
    ...
    if not x or not y then
        x, y = MissionUT.getSector(0, 0, 300, 450, false, false, false, false, false)
    end

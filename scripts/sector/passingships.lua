...
function PassingShips.addPassingBehavior(ship, destination_in, gateId_in)
    ...
    if destination_in or gateId_in then
        gateId = gateId_in
        ...
        if #gates > 0 then
            if random():test(0.25) then
                gateId = randomEntry(gates).id
            end
        end

        if not gateId then
            -- find position where the trader will jump into hyperspace
            destination = -ship.translationf + vec3(math.random(), math.random(), math.random()) * 1000
    ...
    if not gateId then
        ...
        ship:addScriptOnce("ai/passgate.lua", gateId)
    end

    return destination, gateId
end
...
function PassingShips.createPassingTrader(faction)
    ...
    local onFinished = function(ships)
        ...
        for _, ship in pairs(ships) do
            ...
            if not gateId and not destination then
                destination, gateId = PassingShips.addPassingBehavior(ship)
            else
                PassingShips.addPassingBehavior(ship, destination, gateId)
            end
...
function PassingShips.createPassingCruiseShip(faction)
    ...
    -- finalize ship
    PassingShips.addPassingBehavior(ship)
    ship:setValue("passing_ship", true)

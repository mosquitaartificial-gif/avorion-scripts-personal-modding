    ...
    if Sector().numPlayers >= 1 then
        ...
        if data.initialEnemies then
            ...
            if data.welcomeTimer <= 0 then
                ...
                Sector():broadcastChatMessage(enemy, ChatMessageType.Chatter, "Go away, we are waiting for a ... friend."%_t)
    ...
    for _, enemy in pairs(enemies) do
        if enemy:getValue("is_wave") then
            if not data.chatMessageSent then
                Sector():broadcastChatMessage(enemy, ChatMessageType.Chatter, "We told you to leave. Now you will be our first victim."%_t)

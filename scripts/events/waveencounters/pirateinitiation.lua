...
function updateServer(timeStep)
    if Sector().numPlayers >= 1 then
        ...
        if data.welcomeTimerPassed then
            data.attackTimer = data.attackTimer - timeStep
        end

        if data.attackTimer <= 0 and not data.attackTimerPassed then
            startEncounter()
            data.attackTimerPassed = true
        end
        ...
        if data.initialEnemies then
            ...
            if data.welcomeTimer <= 0 then
                ...
                Sector():broadcastChatMessage(enemy, ChatMessageType.Chatter, "Hey! Stay here! We will prove ourselves against you!"%_t)

...
function collectHighlightableObjects()
    ...
    for _, entity in pairs({Sector():getEntitiesByScriptValue("valuable_object")}) do
        ...
        if not entity.dockingParent then
            if type(value) == "string" then
                ...
                objects[entity.id] = {entity = entity, color = Rarity(value).color, tilted = false}
            end
        end
    end
    ...
    for _, entity in pairs({Sector():getEntitiesByScriptValue("blackbox_wreckage")}) do
        -- docked objects are not available for the player
        if not entity.dockingParent then
            objects[entity.id] = {entity = entity, color = ColorRGB(0.3, 0.9, 0.9), tilted = false}
        end
    end
    ...
    for _, entity in pairs({Sector():getEntitiesByComponent(ComponentType.CargoLoot)}) do
        ...
        if loot:matches("Rift Research Data") then
            objects[entity.id] = {entity = entity, color = Rarity(RarityType.Rare).color, tilted = true}
        end
    end
...
function onPreRenderHud()
    ...
    for id, object in pairs(interestingEntities) do
        ...
        if distance2(object.entity.translationf, center) < range2 then
            ...
            local indicator = TargetIndicator(object.entity)
            indicator.color = ColorRGB(c.x, c.y, c.z)
            indicator.size = size + 1.5 * timeFactor
            if object.tilted then
...
function getDescriptionLines(seed, rarity, permanent)
    ...
    table.insert(texts, {ltext = "Object Detection System"%_t})
